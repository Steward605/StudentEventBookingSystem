...
router.get('/', requireAuth, (req, res) => {
  const bookings = db.prepare(`
    SELECT b.*, e.title, e.category, e.location, e.city, e.event_date, e.start_time, e.end_time, e.price, e.image_url
    FROM bookings b
...
router.get('/all', requireAuth, requireAdmin, (req, res) => {
  const bookings = db.prepare(`
    SELECT b.*, u.name AS student_name, u.email AS student_email,
      e.title, e.category, e.location, e.city, e.event_date, e.start_time, e.end_time, e.price, e.image_url
    FROM bookings b
...
router.post('/', requireAuth, (req, res, next) => {
  ...
  try {
    ...
    const bookEventTransaction = db.transaction(() => {
      // check capacity safely inside the locked transaction
      ...
      const result = db.prepare(`
        INSERT INTO bookings (user_id, event_id, ticket_count, attendee_name, attendee_email, booking_reference)
        VALUES (?, ?, ?, ?, ?, ?)
    ...
    // fetch the fully joined booking record to return to the frontend
    const booking = db.prepare(`
      SELECT b.*, e.title, e.location, e.event_date, e.start_time, e.price
      FROM bookings b
      JOIN events e ON e.id = b.event_id
      WHERE b.id = ?
    `).get(newBookingId);
